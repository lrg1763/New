import { FC, useRef, useState } from 'react' // Импортируем FC, useRef и useState из библиотеки React
import { ToastContainer, toast } from 'react-toastify' // Импортируем ToastContainer и toast из библиотеки react-toastify
import { Formik, Form } from 'formik' // Импортируем Formik и Form из библиотеки Formik
import { profileValidationSchema } from '../../../utils/validation' // Импортируем схему валидации профиля
import classNames from 'classnames' // Импортируем библиотеку classnames для условного объединения классов
import styles from './MyProfile.module.scss' // Импортируем стили из файла MyProfile.module.scss
import ProfileField from './ProfileField/ProfileField' // Импортируем компонент ProfileField
import { useActions } from 'src/hooks/useAction' // Импортируем useActions из хуков

// Определение компонента MyProfile с использованием функционального компонента
const MyProfile: FC = () => {
	const [isEdit, setIsEdit] = useState(false) // Состояние для управления режимом редактирования

	const { logoutUser, deleteUser } = useActions() // Получаем действия logoutUser и deleteUser из хуков
	...
	const ref = useRef<HTMLFormElement>(null) // Создаем реф для формы
	const oldUser = useRef({ ...user }) // Создаем реф для хранения старого пользователя
...
export default MyProfile // Экспортируем компонент MyProfile по умолчанию
